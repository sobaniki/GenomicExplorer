...
intensity <- if(!is.null(params$intensity_file)) as.character(params$intensity_file) else ""
have_fitpoly <- requireNamespace("fitPoly", quietly=TRUE)

dosage_out <- file.path(out_dir, "dosage.tsv")
note <- NULL
if (!have_fitpoly) note <- "fitPoly not installed; wrote placeholder dosage."
if (have_fitpoly && nzchar(intensity) && file.exists(intensity)) {
  note <- "fitPoly installed; runner scaffold (implement calling here)."
}
set.seed(1)
...
qc_png <- file.path(out_dir, "cluster_plot.png")
placeholder_plot(qc_png, "Cluster plot (placeholder)")

meta <- list(
  module="poly_fitpoly_call",
  ploidy=ploidy,
  intensity_file=basename(intensity),
  note=note,
  dosage_tsv="dosage.tsv",
  plot="cluster_plot.png",
